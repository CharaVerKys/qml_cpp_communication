...
public:
    ...
    void init(){
        ...
        streams.insert(cvk::log::to::qt, std::make_shared<std::ofstream>(logDir / (time+"_qt.log"),std::ios::app));
        if(errno not_eq 0){
            std::cerr << "error during logging: " << std::strerror(errno) << "\nduring try create qt log file on path: " << logDir.string() <<"\n"; 
        }
        streams.insert(cvk::log::to::appCoutCerr, std::make_shared<std::ofstream>(logDir / (time+"_apOut.log"),std::ios::app));
        if(errno not_eq 0){
            std::cerr << "error during logging: " << std::strerror(errno) << "\nduring try create appCoutCerr file on path: " << logDir.string() <<"\n"; 
        }
        impl.start(streams);
    }
